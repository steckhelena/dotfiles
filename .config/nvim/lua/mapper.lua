...
if is_module_available "nvim-mapper" then
    ...
    M.map =
        function(mode, keys, cmd, options, category, unique_identifier, description)
            mapper.map(
                mode,
                keys,
                cmd,
    ...
    M.map_virtual = function(_, _, _, _, _, _, _) end
    M.map_buf_virtual = function(_, _, _, _, _, _, _) end
end
